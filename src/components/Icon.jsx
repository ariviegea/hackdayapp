import React from 'react';

function Icon() {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      x='0'
      y='0'
      enableBackground='new 0 0 27.02 27.02'
      version='1.1'
      viewBox='0 0 27.02 27.02'
      xmlSpace='preserve'
    >
      <g fill='#030104'>
        <path d='M3.674 24.876s-.024.604.566.604l6.811-.008.01-5.581s-.096-.92.797-.92h2.826c1.056 0 .991.92.991.92l-.012 5.563h6.667c.749 0 .715-.752.715-.752V14.413l-9.396-8.358-9.975 8.358v10.463z'></path>
        <path d='M0 13.635s.847 1.561 2.694 0l11.038-9.338 10.349 9.28c2.138 1.542 2.939 0 2.939 0L13.732 1.54 0 13.635z'></path>
        <path d='M23.83 4.275L21.168 4.275 21.179 7.503 23.83 9.752z'></path>
      </g>
    </svg>
  );
}

export default Icon;